...
onload = function () {
    ...
    setInterval(function () {
        analyser.getByteFrequencyData(dataArray);
        var frAvg = Math.pow(Math.max(.1, 1 - avg(dataArray) / (255)), 2);
        var scale = radius * frAvg * 4;
        var t = Math.sin(Date.now() * .001) * 2 - 1;
        document.title = t;
        ...
        ball.geometry.vertices.forEach(function (vertex, i) {
            ...
            // radius 1 ---- 0
            vertex.normalize();
            var seed = 0.5 * (simplex.noise3D(
            // vertex.x*radius / scale, 
            // vertex.y*radius / scale, 
            // vertex.z*radius / scale ) + 1
            vertex.x * radius * t * .01, vertex.y * radius * t * .01, vertex.z * radius * t * .01) + 1);
